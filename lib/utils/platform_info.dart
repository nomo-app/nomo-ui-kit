import 'dart:io';
import 'package:flutter/foundation.dart' show kIsWeb;

class PlatformInfo {
  final bool isDesktop;
  final bool isCupertino;
  final bool isMaterial;
  final bool isWeb;

  static PlatformInfo? _instance;

  const PlatformInfo({
    required this.isWeb,
    this.isDesktop = false,
    this.isCupertino = false,
    this.isMaterial = false,
  });

  static PlatformInfo get I {
    return _instance ??= kIsWeb
        ? const PlatformInfo(isWeb: true)
        : PlatformInfo(isWeb: false, isCupertino: _isCupertino, isDesktop: _isDesktop, isMaterial: _isMaterial);
  }

  static bool get _isCupertino => Platform.isIOS || Platform.isMacOS;

  static bool get _isMaterial => !_isCupertino;

  static bool get _isDesktop => Platform.isLinux || Platform.isMacOS || Platform.isMacOS;
}
